...
def scrape_jobstreet(
    ...
    country="id",
    page=1,
    cookies_file="app/config/jobstreet.json",
):
    country_urls = {
    ...
    try:
        ...
        total_jobs_tag = soup.find(
            "h1", id="SearchSummary", attrs={"data-automation": "totalJobsMessage"}
        )
        total_jobs = (
            total_jobs_tag.find(
                "span", attrs={"data-automation": "totalJobsCount"}
            ).get_text(strip=True)
            if total_jobs_tag
            else "0"
        )
        ...
        for job_card in job_cards:
            ...
            company_name_tag = job_card.find(
                "a", attrs={"data-automation": "jobCompany"}
            )
            company_name = (
                company_name_tag.get_text(strip=True) if company_name_tag else "N/A"
            )

            # Ekstrak logo perusahaan
            company_logo_container = job_card.find(
                "div", attrs={"data-automation": "company-logo-container"}
            )
            company_logo_tag = (
                company_logo_container.find("img") if company_logo_container else None
            )
            company_logo = (
                company_logo_tag["src"]
                if company_logo_tag and company_logo_tag.has_attr("src")
                else "N/A"
            )
            ...
            # Ekstrak lokasi
            location_tags = job_card.find_all(
                "a", attrs={"data-automation": "jobLocation"}
            )
            location = (
                ", ".join([tag.get_text(strip=True) for tag in location_tags])
                if location_tags
                else "N/A"
            )
            ...
            link_tag = job_card.find(
                "a", href=True, attrs={"data-automation": "jobTitle"}
            )
            link = (
                f"https://{country_urls[country].split('/')[2]}{link_tag['href']}"
                if link_tag
                else "N/A"
            )
        ...
        if pagination:
            ...
            page_tags = pagination.find_all(
                "a", attrs={"data-automation": re.compile(r"page-\d+")}
            )
            if page_tags:
                last_page_numbers = [
                    int(tag.get_text(strip=True))
                    for tag in page_tags
                    if tag.get_text(strip=True).isdigit()
                ]
        ...
        suggestion_location = []
        if suggestion_span:
            suggestion_links = suggestion_span.find_all(
                "a", attrs={"data-automation": re.compile(r"didYouMeanLocation\d+")}
            )
            for link in suggestion_links:
                suggestion_location.append(
                    {
                        "location_name": link.get_text(strip=True),
                        "url": f"https://{country_urls[country].split('/')[2]}{link['href']}"
