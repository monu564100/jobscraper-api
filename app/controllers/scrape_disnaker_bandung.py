...
def scrape_disnaker_bandung(page='1'):
    ...
    try:
        ...
        for job in job_elements:
            ...
            if not (logo == 'N/A' and company == 'N/A' and title == 'N/A' and location == 'N/A' and date == 'N/A'):
                results.append({
                    'logo': logo,
                    'company': company,
                    'title': title,
        ...
        if pagination_tag:
            ...
            page_numbers = [
                int(item.get_text(strip=True)) for item in page_items if item.get_text(strip=True).isdigit()
            ]
            total_pages = max(page_numbers) if page_numbers else 0

        # Validasi jika halaman sudah berakhir
        if showing_start == 0 and showing_end == 0 and total_pages == 0 and total_results == 0:
            return ResponseHelper.success_response('No more data available.', {
                'jobs': [],
                'total_results': 0,
                'showing_start': 0,
                'showing_end': 0,
        ...
        return ResponseHelper.success_response('Success scraping Disnaker Bandung jobs', {
            'jobs': results,
